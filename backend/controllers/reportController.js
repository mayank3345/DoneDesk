...
const exportTasksReports = async (req, res) => {
  try {
    ...
    worksheet.columns = [
      ...
      { header: "Assigned To", key: "assignedTo", width: 30 }, // fixed key name
    ];
...
//@desc  Export users-tasks as an Excel File
...
const exportUsersReports = async (req, res) => {
  try {
    ...
    // Build map of users with task stats
    const userTaskMap = {};
    users.forEach((user) => {
    ...
    const workbook = new excelJS.Workbook(); // FIXED: added workbook
    const worksheet = workbook.addWorksheet("User Task Report"); // FIXED: added worksheet
